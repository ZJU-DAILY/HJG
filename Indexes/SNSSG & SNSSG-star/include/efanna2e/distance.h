...
namespace efanna2e{
  enum Metric{
    ...
    public:
        ...
        virtual float compare(const string a, const string b, unsigned length) const = 0;
        virtual ~Distance() {}
    };
...
//nomal distance
      ...
      while (a < last) {
        ...
        float compare(const string a, const string b, unsigned size) const {return 0;}
    };
...
#define SSE_L2NORM(addr, dest, tmp) \
  ...
  #define AVX_L1SQR(addr1, addr2, dest, tmp1, tmp2) \
      tmp1 = _mm256_loadu_ps(addr1);\
      tmp2 = _mm256_loadu_ps(addr2);\
      tmp1 = _mm256_sub_ps(tmp1, tmp2); \
      tmp1 = _mm256_mul_ps(tmp1, tmp1); \
...
#else
#ifdef __SSE2__
  #define SSE_L1SQR(addr1, addr2, dest, tmp1, tmp2) \
          tmp1 = _mm_load_ps(addr1);\
          tmp2 = _mm_load_ps(addr2);\
          tmp1 = _mm_sub_ps(tmp1, tmp2); \
          tmp1 = _mm_mul_ps(tmp1, tmp1); \
...
//nomal distance
      ...
      while (a < unroll_group) {
          ...
          diff2 = a[2] - b[2];
          diff3 = a[3] - b[3];
          result += abs(diff0) + abs(diff1) + abs(diff2) + abs(diff3);
          a += 4;
          b += 4;
      }
      /* Process last 0-3 pixels.  Not needed for standard vector lengths. */
    ...
    class DistanceLDistance : public Distance{
    public:
        float compare(const string a, const string b, unsigned size) const {
        	string a_process = a;
        	string b_process = b;
        	
        	//a_process.erase(std::remove(a_process.begin(), a_process.end(), '\n'), a_process.end());
        	//b_process.erase(std::remove(b_process.begin(), b_process.end(), '\n'), b_process.end());
        
        	//string a_process = a.substr(0, a.length() - 1);
        	//string b_process = b.substr(0, b.length() - 1);
        	
        	float result = 0;
            	int n = a_process.length();
            	int m = b_process.length();
            	
            	if (n == 0){return (float)m;}
		if (m == 0){ return (float)n;}
            
		int** table1 = new int* [n + 1];
		for (int i = 0; i < n + 1; i++) table1[i] = new int[m + 1];
		for (int i = 0; i <= n; i++) table1[i][0] = i;
		for (int j = 0; j <= m; j++) table1[0][j] = j;
		for (int i = 1; i <= n; i++) {
			for (int j = 1; j <= m; j++) {
				int cost = (a_process[i - 1] == b_process[j - 1]) ? 0 : 1;
        ...
        float compare(const float* a, const float* b, unsigned size) const {return 0;}
    };
    
    class DistanceCosDis : public Distance{
    public:
        float compare(const float* a, const float* b, unsigned size) const {
            float result = 0;
  ...
  #define AVX_CosDisSQR(addr1, addr2, dest, tmp1, tmp2) \
      tmp1 = _mm256_loadu_ps(addr1);\
      tmp2 = _mm256_loadu_ps(addr2);\
      tmp1 = _mm256_sub_ps(tmp1, tmp2); \
      tmp1 = _mm256_mul_ps(tmp1, tmp1); \
...
#else
#ifdef __SSE2__
  #define SSE_CosDisSQR(addr1, addr2, dest, tmp1, tmp2) \
          tmp1 = _mm_load_ps(addr1);\
          tmp2 = _mm_load_ps(addr2);\
          tmp1 = _mm_sub_ps(tmp1, tmp2); \
          tmp1 = _mm_mul_ps(tmp1, tmp1); \
...
//nomal distance
      ...
      const float* last = a + size;
      const float* unroll_group = last - 3;
      double sa1 = 0, sa2 = 0, sa3 = 0, dis = 0;

      /* Process 4 items with each loop for efficiency. */
      while (a < unroll_group) {
          sa1 += a[0] * a[0] + a[1] * a[1] + a[2] * a[2] + a[3] * a[3];
          sa2 += b[0] * b[0] + b[1] * b[1] + b[2] * b[2] + b[3] * b[3];
          sa3 += a[0] * b[0] + a[1] * b[1] + a[2] * b[2] + a[3] * b[3];
