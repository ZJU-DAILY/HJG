...
void IndexGraph::joinS() {
#pragma omp parallel for default(shared) schedule(dynamic, 100)
  for (unsigned n = 0; n < nd_; n++) {
    graph_[n].join([&](unsigned i, unsigned j) {
      if(i != j){
        float dist = distance_->compare(data_2[i * dimension_], data_2[j * dimension_], dimension_);
        graph_[i].insert(j, dist);
        graph_[j].insert(i, dist);
      }
    });
...
void IndexGraph::NNDescentS(const Parameters &parameters) {
  unsigned iter = parameters.Get<unsigned>("iter");
  std::mt19937 rng(rand());
  std::vector<unsigned> control_points(_CONTROL_NUM);
  std::vector<std::vector<unsigned> > acc_eval_set(_CONTROL_NUM);
  GenRandom(rng, &control_points[0], control_points.size(), nd_);
  generate_control_setS(control_points, acc_eval_set, nd_);
  for (unsigned it = 0; it < iter; it++) {
    joinS();
    update(parameters);
    //checkDup();
...
void IndexGraph::generate_control_setS(std::vector<unsigned> &c,
                                      std::vector<std::vector<unsigned> > &v,
                                      unsigned N){
#pragma omp parallel for
  for(unsigned i=0; i<c.size(); i++){
    std::vector<Neighbor> tmp;
    for(unsigned j=0; j<N; j++){
      float dist = distance_->compare(data_2[c[i] * dimension_], data_2[j * dimension_], dimension_);
      tmp.push_back(Neighbor(j, dist, true));
    }
    std::partial_sort(tmp.begin(), tmp.begin() + _CONTROL_NUM, tmp.end());
    for(unsigned j=0; j<_CONTROL_NUM; j++){
...
void IndexGraph::InitializeGraphS(const Parameters &parameters) {
...
#pragma omp parallel for
  for (unsigned i = 0; i < nd_; i++) {
    //const float *query = data_ + i * dimension_;
    string query = data_2[i * dimension_];
    std::vector<unsigned> tmp(S + 1);
    initializer_->Search(query, data_2, S + 1, parameters, tmp.data());

    for (unsigned j = 0; j < S; j++) {
      unsigned id = tmp[j];
      if (id == i)continue;
      float dist = distance_->compare(data_2[i * dimension_], data_2[id * dimension_], (unsigned) dimension_);
...
void IndexGraph::InitializeGraph_Refine(const Parameters &parameters) {
	std::cout<<"final_graph_.size(): "<<final_graph_.size()<<std::endl;
...
void IndexGraph::Build(size_t n, const vector<string> data, const Parameters &parameters){
	//assert(initializer_->GetDataset() == data);
  data_2 = data;
  assert(initializer_->HasBuilt());

  InitializeGraphS(parameters);
  NNDescentS(parameters);
...
void IndexGraph::Search(
  ...
  for(unsigned i=0; i<L; i++){
    unsigned id = init_ids[i];
    float dist = distance_->compare(data_2[dimension_*id], query, (unsigned)dimension_);
    retset[i]=Neighbor(id, dist, true);
  }
  ...
  while(k < (int)L) {
    ...
    if (retset[k].flag) {
      ...
      for (unsigned m = 0; m < final_graph_[n].size(); ++m) {
        ...
        if(flags[id])continue;
        flags[id] = 1;
        float dist = distance_->compare(query, data_2[dimension_ * id], (unsigned)dimension_);
        if(dist >= retset[L-1].distance)continue;
        Neighbor nn(id, dist, true);
        int r = InsertIntoPool(retset.data(), L, nn);
...
void IndexGraph::Save(const char *filename) {
  ...
  assert(final_graph_.size() == nd_);
  out.write((char *) &pivot, sizeof(unsigned));
...
void IndexGraph::SaveGraph(const char *filename, vector<unsigned> mapToId) {
  std::ofstream out(filename, std::ios::binary | std::ios::out);
  assert(final_graph_.size() == nd_);
  
  out.write((char *) &pivot, sizeof(unsigned));
  out.write((char *) mapToId.data(), nd_ * sizeof(unsigned));
  unsigned GK = (unsigned) final_graph_[0].size();
  for (unsigned i = 0; i < nd_; i++) {
    out.write((char *) &GK, sizeof(unsigned));
    out.write((char *) final_graph_[i].data(), GK * sizeof(unsigned));
...
void IndexGraph::setPivot(unsigned p){
	this->pivot = p;
}
